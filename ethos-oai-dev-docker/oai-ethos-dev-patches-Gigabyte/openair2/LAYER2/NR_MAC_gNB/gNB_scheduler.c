...
void gNB_dlsch_ulsch_scheduler(module_id_t module_idP, frame_t frame, sub_frame_t slot, NR_Sched_Rsp_t *sched_info)
{
  ...
  if (true) {            // ETHOS: per slot frequency
    NR_UEs_t *UE_info = &RC.nrmac[module_idP]->UE_info;
    update_with_rl(frame, slot, UE_info->list);
  }
  print_rw_stats(frame, slot);
  // ---------------------------------------------------------//
  ...
  if ((slot == 0) && (frame & 127) == 0) {
    ...
    char log_buffer[512];
    print_meas_log(&gNB->schedule_dlsch,"ethos_dlsch",NULL,NULL, log_buffer, sizeof(log_buffer));
    LOG_I(NR_MAC, "%s", log_buffer);
  }
  NR_UEs_t *UE_info = &RC.nrmac[module_idP]->UE_info;
  update_ethos_ue_metric_reg(frame, slot, UE_info->list);
  ...
  if ((slot == 0) && (frame & 127) == 0) {
    ...
    cpumeas(CPUMEAS_ENABLE);
    char log_buffer[512];
    print_meas_log(&gNB->eNB_scheduler,"gNB_scheduler",NULL,NULL, log_buffer, sizeof(log_buffer));
    LOG_I(NR_MAC, "%s", log_buffer);
  }
