                       ...
                       const uint16_t rssi)
{
  ...
  if (UE && UE_waiting_CFRA_msg3 == false) {
    ...
    LOG_D(NR_MAC, "[gNB %d][PUSCH %d] CC_id %d %d.%d Received ULSCH sdu from PHY (rnti %04x) ul_cqi %d TA %d sduP %p, rssi %d\n",
          ...
          sduP,
          rssi);
    // ethos --- added from 1.6 integration
    UE_scheduling_control->CSI_report.ul_cqi = ul_cqi;

    // if not missed detection (10dB threshold for now)
    if (rssi>0) {
      UE->mac_stats.NPRB = UE_scheduling_control->ul_harq_processes[harq_pid].sched_pusch.rbSize;       // ethos added from 1.6 integration
      ...
      // --- ethos added for 1.6 integration ---
                  ...
                  uint16_t *rballoc_mask)
{
  ...
  UE_iterator(UE_list, UE) {
    ...
    uint8_t mcs_result = get_ul_mcs_of_current_ue(UE->rnti);
    if (mcs_result){
          sched_pusch->mcs = mcs_result;
    ...
    if (B == 0 && do_sched) {
      ...
      uint8_t nrOfLayers_result = get_ul_nrOfLayers_of_current_ue(UE->rnti);
      if (nrOfLayers_result){
        sched_pusch->nrOfLayers = nrOfLayers_result;
      }
    
      if (sched_pusch->nrOfLayers > 0) {
        UE->ethos_metric_reg.ul_nrOfLayers = sched_pusch->nrOfLayers;
      }
  ...
  while (remainUEs> 0 && n_rb_sched >= min_rb && iterator->UE != NULL) {
    ...
    uint8_t nrOfLayers_result = get_ul_nrOfLayers_of_current_ue(iterator->UE->rnti);
    if (nrOfLayers_result){
      sched_pusch->nrOfLayers = nrOfLayers_result;
    }

    if (sched_pusch->nrOfLayers > 0) {
      iterator->UE->ethos_metric_reg.ul_nrOfLayers = sched_pusch->nrOfLayers;
    }
    sched_pusch->time_domain_allocation = get_ul_tda(nrmac, scc, sched_pusch->frame, sched_pusch->slot);
    sched_pusch->tda_info = get_ul_tda_info(current_BWP,
                                            sched_ctrl->coreset->controlResourceSetId,
    ...
    float ul_rbWeight = get_ul_rbWeight_of_current_ue(iterator->UE->rnti);
    if (ul_rbWeight > 0){
      sched_pusch->rbWeight = ul_rbWeight;
      sched_pusch->max_weighted_rbSize = (uint16_t) (ul_rbWeight*current_BWP->BWPSize);
    } else {
      sched_pusch->rbWeight = 1.0f;
      sched_pusch->max_weighted_rbSize = current_BWP->BWPSize;
    }
    max_rbSize = min(max_rbSize, sched_pusch->max_weighted_rbSize);
