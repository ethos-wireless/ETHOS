...
typedef struct NR_sched_pusch {
  ...
  /// ETHOS max weighted rbSize info
  float rbWeight;
  uint16_t max_weighted_rbSize;
} NR_sched_pusch_t;
...
typedef struct NR_mac_stats {
  ...
  int pusch_snrx10;   // ethos -- added from 1.6 integration
  int deltaMCS;       // ethos -- added from 1.6 integration
  int NPRB;           // ethos -- added from 1.6 integration
} NR_mac_stats_t;
...
/*! \brief added struct for keeping valid data for some UE metrics*/
typedef struct{
  ...
  double last_timestamp_us;                            // used external -- update every shmwrite
  uint64_t last_dl_total_bytes;                       // used external -- update every shmwrite
  uint64_t last_ul_total_bytes;                       // used external -- update every shmwrite
  uint16_t dl_rbSize_log;
  uint16_t ul_rbSize_log;
}ue_metric_reg_t;
/*---------------------------------------------------*/
...
typedef struct {
  ...
  // ethos metric reg per slot
  ue_metric_reg_t ethos_metric_reg;
} NR_UE_info_t;
