                  ...
                  uint16_t *rballoc_mask)
{
  ...
  UE_iterator(UE_list, UE) {
    ...
    if (sched_pdsch->dl_harq_pid >= 0) {
      ...
      LOG_D(NR_MAC, "ethos pf_dl before lookup mcs and nrOfLayers[UE_rnti %04x][%4d.%2d] b %d, thr_ue %f, mcs %u, stats_mcs %u, nrOfLayers %u\n",
            UE->rnti,
            frame,
            slot,
            b,
            UE->dl_thr_ue,
            sched_pdsch->mcs, sched_ctrl->dl_bler_stats.mcs, sched_pdsch->nrOfLayers);
      if (true) {
      uint8_t mcs_result = get_dl_mcs_of_current_ue(UE->rnti);                 
      uint8_t nrOfLayers_result = get_dl_nrOfLayers_of_current_ue(UE->rnti);      
      float dl_rbWeight = get_dl_rbWeight_of_current_ue(UE->rnti); 
      if (mcs_result){
      sched_pdsch->mcs = mcs_result;
      sched_ctrl->dl_bler_stats.mcs = sched_pdsch->mcs;
      }
      if (nrOfLayers_result){
      sched_pdsch->nrOfLayers = nrOfLayers_result;
      }
      if (dl_rbWeight > 0){
        sched_pdsch->rbWeight = dl_rbWeight;
        sched_pdsch->max_weighted_rbSize = (uint16_t) (dl_rbWeight*current_BWP->BWPSize);
      } else {
        sched_pdsch->rbWeight = 1.0f;
        sched_pdsch->max_weighted_rbSize = current_BWP->BWPSize;
      }
      LOG_D(NR_MAC, "ethos pf_dl after lookup mcs and nrOfLayers[UE_rnti %04x][%4d.%2d] b %d, thr_ue %f, mcs %u, stats_mcs %u, nrOfLayers %u, mcs_result %u, nrOfLayers_result %u\n",
      UE->rnti,
      frame,
      slot,
      b,
      UE->dl_thr_ue,
      sched_pdsch->mcs, sched_ctrl->dl_bler_stats.mcs, sched_pdsch->nrOfLayers, mcs_result, nrOfLayers_result);
      }
  ...
  while (remainUEs> 0 && n_rb_sched >= min_rbSize && iterator->UE != NULL) {
    ...
    * allocation after CCE alloc fail would be more complex) */
    ...
    max_rbSize = min(max_rbSize, sched_pdsch->max_weighted_rbSize);
    ...
    iterator->UE->ethos_metric_reg.dl_rbSize_log = sched_pdsch->rbSize;
